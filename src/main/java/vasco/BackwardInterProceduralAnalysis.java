...
public abstract class BackwardInterProceduralAnalysis<M,N,A> extends InterProceduralAnalysis<M,N,A> {
		...
		while (!worklist.isEmpty()) {
			...
			if (node != null) {
				...
				if (in.equals(prevIn) == false) {
					// Then addLoop predecessors to the work-list.
				...
				// path to the start of the method, so addLoop the NULL unit
				...
				// Entry value is the copyWithMerge of the INs of the head nodes.
	 ...
	 * <li>Initialise IN/OUT for all nodes and addLoop them to the work-list</li>
	...
	protected Context<M,N,A> initContext(M method, A exitValue) {
		...
		// Initialise IN/OUT for all nodes and addLoop them to the work-list
